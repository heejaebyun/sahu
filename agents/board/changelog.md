...
### 수익 모델 변경 (4/2)
- **건당 CPA/매칭 수수료 모델 전면 폐기** — 변호사법 제34조 (사건 브로커 금지) 위반 확인
- 전문가 연결 수익은 **월정액 배너 광고 모델**로만 (월 30만원~/곳)
- ROADMAP.md 수익 시뮬레이션 전면 수정

### 결제 플로우 확정 (4/2)
- 결제 수단: 카카오페이 + 토스 송금 + 계좌 문자안내 3트랙
- 입금 매칭: 입금자 실명 기준 (주문번호 메모 강요 안 함)
- Apps Script Installable Trigger → Vercel API → PDF 렌더링 → 이메일 발송 자동화
- Vercel SAHU_ADMIN_SECRET 환경변수 + API 헤더 검증 필수
- Apps Script 스크립트 속성에 키 저장 (하드코딩 금지)
