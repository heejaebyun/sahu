      ...
      <Page size="A4" style={styles.page}>
        ...
        {LIFE_CATEGORIES.map((cat) => {
          ...
          const filledItems = items.filter((item) => {
            ...
            return arr.some((entry) => item.fields.some((f) => entry[f.key]?.trim?.()));
          });
          ...
          return (
            <View key={cat.id}>
              ...
              {filledItems.map((item) => {
                ...
                return (
                  <View key={item.id}>
                    <Text style={styles.itemTitle}>{item.title} ({arr.length}개)</Text>
                    {arr.map((entry, ei) => (
                      <View key={ei}>
                        {arr.length > 1 && <Text style={{ fontSize: 9, color: "#999", marginTop: 4 }}>#{ei + 1}</Text>}
